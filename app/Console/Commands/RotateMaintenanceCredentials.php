...
class RotateMaintenanceCredentials extends Command
{
    protected $signature = 'maintenance:rotate
    ...
    protected $description = 'Generate a new maintenance-mode passphrase and record the rotation.';
    ...
    public function handle(): int
    {
        ...
        $username = (string) config('maintenance.login');

        if (! $this->option('no-audit')) {
            AuditLogger::record(
                event: 'maintenance_credentials_rotated',
        ...
        if ($this->option('update-env')) {
            ...
            $this->info("Maintenance passphrase updated in .env ({$length} chars).");
            $this->warn('Store the passphrase in the enterprise vault and sealed envelope immediately.');

            return self::SUCCESS;
        }

        $this->info("Maintenance username : {$username}");
        $this->info("New passphrase (length {$length}):");
        $this->info('    '.$newPassword);
        $this->warn('');
        $this->warn('Set MAINTENANCE_PASSWORD in the production .env and store this value securely.');
        $this->warn('Rotate again using: php artisan maintenance:rotate --update-env');
    ...
    private function updateEnvFile(string $newPassword): void
    {
        ...
        $line = 'MAINTENANCE_PASSWORD='.$newPassword;

        if (preg_match('/^MAINTENANCE_PASSWORD=.*$/m', $contents)) {
            $contents = (string) preg_replace(
                '/^MAINTENANCE_PASSWORD=.*$/m',
                $line,
                $contents
            );
        } else {
